...
namespace ConsoleApp107
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Liste.OrganiseMeeting(perosoane[pers_random], perosoane[pers_random + 2], perosoane[pers_random + 10]);
            Console.ReadKey();
        }
    }
    ...
    static class Liste
    {
        ...
        public static Activity OrganiseMeeting(params Date_pers[] pers)
        {
            Activity meeting = new Activity() { Nume = "Meeting", Descriere = "" };
            meeting.Participanti = pers.ToList();

            DateTime durata_ = new DateTime();
            foreach (var persona in pers)
            {
                if (persona.Agenda != null)

                    foreach (var activitate in persona.Agenda.Activitati)
                    {

                        if (activitate.Sfarsit > durata_) durata_ = activitate.Sfarsit;
                    }
            }

            meeting.Inceput = durata_;
            meeting.Sfarsit = durata_.AddHours(2);


            foreach (var person in pers)
                person.ActivityPlus(meeting);
